...
function canAccessReport(report, user) {
  ...
  if (report.accessLevel === 'premium')
    return [ROLES.ADMIN, ROLES.EDITOR, ROLES.ANALYST].includes(user.role)
  return false
}
...
const getReport = asyncHandler(async (req, res) => {
  ...
  const responseReport = accessible ? report : { ...report, body: null, pdfUrl: null }
...
const updateReport = asyncHandler(async (req, res) => {
  const report = await Report.findOneAndUpdate({ slug: req.params.slug }, req.body, {
    new: true,
    runValidators: true,
  })
  if (!report) throw AppError.notFound('Report')
